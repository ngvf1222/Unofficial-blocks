const LibraryCreator = {
  start: (blocksJSON, category, text) => {
    ...
    if (typeof useWebGL == "undefined") {
      ...
      // 아이콘 적용
      $('head').append(`<style>#entryCategory${category}{background-image:url(https://search.pstatic.net/common/?src=http%3A%2F%2Fblogfiles.naver.net%2F20160222_233%2Fj_dh1991_1456106505134qdUgn_JPEG%2FB1A9BFEE_B0EDBEE7C0CC_BBE7C1F8_BDBAC4DA%253FBDAC_C6FAB5E5_7.jpg&type=sc960_832);background-repeat:no-repeat;margin-bottom:1px;background-position-y: 10px;background-size: 20px;}.entrySelectedCategory#entryCategory${category}{background-image:url(https://raw.githack.com/thoratica/EntBlocks/master/other_selected.svg);background-color:#007bff;border-color:#006ce0;color:#fff}</style>`)
      // 카테고리 이름 적용
      $(`#entryCategory${category}`).append(text)
    }
    console.log('%c냥냥블럭 v0.1%c\n\n냥냥블럭는 MIT License를 따른다냥.\n%c\n공식 블로그:\n%c없다냥~~', 'background-color: #007bff; color: #e9ecef; padding: 0 3rem; border-radius: 1.2rem 0; font-family: sans-serif; font-size: 2.5rem;', 'font-family: sans-serif; font-size: 1.2rem;', 'font-family: sans-serif; font-size: 1.2rem;', 'font-family: sans-serif; font-size: 1.1rem; color: #007bff;')
  }
}
let blockPOST
const blocks = [
  ...
  },
  {
    ...
    def: [
      ...
      {
        type: 'text',
        params: [`{ "images": [], "category": "free", "title": "엔트리봇", "content": "사랑스럽다냥~", "groupNotice": false }`]
      },
      null
    ],
    map: {
    ...
    class: 'text',
    func: async (sprite, script) => {
      if (confirm(`"올리기(POST) 요청" 을 허용하시겠냥?\n내용: ${script.getValue('DATA', script)}`)) {
        let res = await fetch(script.getValue('APIURL', script), {
          method: 'POST',
  ...
  {
    name: 'toast',
    template: '%1 제목과 %2 내용의 %3 토스트를 %4 출력하기다냥%5',
    skeleton: 'basic',
    color: {
      default: '#007bff',
      darken: '#006ce0'
  ...
  {
    name: 'popup',
    template: '%1 제목과 %2 내용의 %3 테마 팝업 출력하기다냥%4',
    skeleton: 'basic',
    color: {
      default: '#007bff',
      darken: '#006ce0'
  ...
  {
    name: 'console',
    template: '%1 내용을 브라우저 콘솔에 %2 하기다냥%3',
    skeleton: 'basic',
    color: {
      default: '#007bff',
      darken: '#006ce0'
  ...
  {
    name: 'entryConsole',
    template: '%1 내용을 엔트리 콘솔에 출력하기다냥%2',
    skeleton: 'basic',
    color: {
      default: '#007bff',
      darken: '#006ce0'
  ...
  {
    name: 'entryUndefined',
    template: '엔트리 변수 %1의 값이 정의되지 않았냥?',
    skeleton: 'basic_boolean_field',
    color: {
      default: '#007bff',
      darken: '#006ce0'
  ...
  },
  {
    ...
    class: 'text',
    func: async (sprite, script) => {
      if (confirm(`"새 탭에서 웹사이트 열기" 를 허용하시겠냥?\nURL: ${script.getValue('URL', script)}`)) {
